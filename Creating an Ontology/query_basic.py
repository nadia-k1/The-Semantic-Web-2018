import logging
import rdflib

# configuring logging
logging.basicConfig()

# creating the graph
g=rdflib.Graph()
result=g.parse("Ontology2.owl", "xml")
print("graph has %s statements.\n" % len(g))

# movies filmed in Hong Kong
query1 = """
PREFIX movie: <http://www.semanticweb.org/ontologies/2018/Ontology1.owl#>
PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>  
SELECT DISTINCT ?name ?country
WHERE { ?film rdf:type movie:Movie .
        	?film movie:movieName ?name .
        	?film movie:filmedIn ?country .
	FILTER regex(str(?country), "Hong_Kong")   
      }
"""

# querying and displaying the results
results = g.query(query1)
print ('{0:30s} {1:40s}'.format("Name","Country"))
print ('{0:30s} {1:40s}'.format("------------------------------","------------------------------------------"))
for name,country in results:
	print ('{0:30s} {1:40s}'.format(name,country))
	
print ("\n\n")

# movies lasting less than 100 minutes
query2 = """
PREFIX movie: <http://www.semanticweb.org/ontologies/2018/Ontology1.owl#>
PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>  
PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
SELECT DISTINCT ?name ?runtime
WHERE { ?film rdf:type movie:Movie .
        	?film movie:movieName ?name .
        	?film movie:runtime ?runtime .
	FILTER (xsd:integer(?runtime) < 100)   
      }
"""
# querying and displaying the results
results = g.query(query2)
print ('{0:30s} {1:30s}'.format("Name","Duration (minutes)"))
print ('{0:30s} {1:30s}'.format("------------------------------","------------------------------"))
for name,duration in results:
	print ('{0:30s} {1:30s}'.format(name,duration))
	
